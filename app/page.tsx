...
export default function Home() {
  ...
  // دالة موحدة لتنظيف الروابط من الذاكرة العشوائية لمنع الـ Memory Leaks
  const revokeUrls = () => {
  ...
  // تنظيف الذاكرة عند إغلاق الصفحة أو المكون
  useEffect(() => {
    return () => revokeUrls();
  }, []);

  // التعامل مع رفع الصورة وتحديد نوع الـ Event بدقة لـ TypeScript
  ...
  // معالجة الصورة لزيادة التباين والحدة قبل إرسالها للذكاء الاصطناعي (تحسين الحواف)
  const preprocessImage = async (file: File): Promise<Blob> => {
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        // تطبيق الفلاتر لتحسين التعرف على الحواف
  ...
  // الدالة الأساسية لإزالة الخلفية محلياً في المتصفح
  const removeBackground = async () => {
    ...
    try {
      ...
      // تم تصحيح الكلمة الإملائية هنا من finaly إلى finally
      setLoading(false);
    }
  };

  // إعادة تعيين التطبيق للحالة الافتراضية
  const resetApp = () => {
    revokeUrls();
    setImage(null);
    setImageFile(null);
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-8">
      ...
      <main className="max-w-6xl mx-auto px-4 pb-24">
        <div className="bg-slate-900/80 backdrop-blur border border-slate-800 p-6 md:p-8 rounded-3xl shadow-2xl flex flex-col items-center gap-8">
          ...
          {/* صندوق رفع الصورة (يظهر فقط إذا لم يتم اختيار صورة) */}
          {!image && (
            <label className="w-full max-w-3xl aspect-[16/9] border-2 border-dashed border-slate-700 hover:border-indigo-500 bg-slate-950 hover:bg-black rounded-3xl flex flex-col items-center justify-center p-6 text-center cursor-pointer transition-all duration-300 group">
              ...
              <p className="text-xl font-semibold mb-2 text-slate-200">
                Upload an image
              </p>

              <p className="text-sm text-slate-500">
                PNG, JPG, WEBP • Local AI Processing
              </p>
            </label>
          )}

          {/* لوحة العرض والتحكم بالنتائج (تظهر بعد اختيار الصورة) */}
          {image && (
            <div className="w-full flex flex-col gap-7">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 w-full">
                
                {/* واجهة الصورة الأصلية */}
                <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 relative">
                  <span className="absolute top-5 left-5 bg-black/70 px-3 py-1 rounded-md text-xs font-semibold text-slate-400">
                    Original Image
                  </span>
                ...
                {/* واجهة النتيجة المعالجة */}
                <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 relative min-h-[300px] flex items-center justify-center">
                  <span className="absolute top-5 left-5 bg-black/70 px-3 py-1 rounded-md text-xs font-semibold text-indigo-400">
                    Processed Result
                  </span>

                  {/* أزرار تغيير الثيم الخاص بالمعاينة (فاتح / غامق) */}
                  {result && !loading && (
                  ...
                  {/* شاشة التحميل ومؤشر التقدم والنسبة المئوية */}
                  {loading && (
                  ...
                  {/* الحالة الافتراضية قبل بدء المعالجة */}
                  {!loading && !result && (
                    <div className="text-center p-6 text-slate-600 flex flex-col items-center gap-3">
                      <ImageIcon size={42} />
                      <p className="text-sm">
                  ...
                  {/* عرض الصورة النهائية بدون خلفية */}
                  {result && !loading && (
              ...
              {/* أزرار التحكم السفلى (إعادة تعيين / معالجة / تحميل) */}
              <div className="flex flex-wrap items-center justify-center gap-4 border-t border-slate-800 pt-7">
                <button
                  onClick={resetApp}
                  disabled={loading}
                  className="flex items-center gap-2 px-5 py-3 bg-slate-800 text-slate-300 rounded-xl font-medium transition hover:bg-slate-700 disabled:opacity-50"
                >
                  <Trash2 size={18} />
                  Reset
                </button>
                ...
                {!result && (
                  <button
                    ...
                    disabled={loading}
                    className="flex items-center gap-2 px-8 py-3 bg-indigo-600 text-white rounded-xl font-bold transition hover:bg-indigo-500 shadow-lg shadow-indigo-600/20 disabled:opacity-50"
                  >
                    <Sparkles size={18} />
                ...
                {result && (
                  <a
                    ...
                    className="flex items-center gap-2 px-8 py-3 bg-emerald-600 text-white rounded-xl font-bold transition hover:bg-emerald-500"
                  >
                    <Download size={18} />
                    Download PNG
                  </a>
